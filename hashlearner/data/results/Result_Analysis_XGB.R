...
xg_boost_train = function(training,testing,nround = 25){
  ...
  for(expected_name in EXPECTED_NAMES){
    ...
    test_label = as.numeric(unlist(testing[expected_name]))
    
    #bstSparse <- xgboost(data = raw_training, label = label, max.depth = 2, eta = 1, nthread = 2, nround = nround, objective = "binary:logistic")
    dtrain = xgb.DMatrix(data = raw_training, label=label)
    dtest = xgb.DMatrix(data = raw_testing, label=test_label)
    
    watchlist <- list(train=dtrain, test=dtest)
    
    bstSparse <- xgb.train(data=dtrain,label = label, booster = "gblinear", max.depth=2, nthread = 2, nround=2, watchlist=watchlist, eval.metric = "error", eval.metric = "logloss", objective = "binary:logistic")
    xg_list[i] = list(bstSparse)
    print(paste("trained",i))
...
main = function(){
  ...
  xg_list = xg_boost_train(training, testing, nround = 25)
